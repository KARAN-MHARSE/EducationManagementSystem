...
import com.aurionpro.ems.services.DashboardService;
...
public class DashboardController {
	...
	public void printDashBoard() {
		List<Dashboard> dashboards = dashboardService.getDashboardData();
		...
		System.out.printf("%-5s %-10s %-20s %-30s %-30s %-30s %-10s %-10s %-10s%n", "SrNo", "StudID", "Student Name",
				"Courses", "Subjects", "Teachers", "Total", "Paid", "Pending");
		System.out.println(
				"--------------------------------------------------------------------------------------------------------------------------------------------------------------------");
			...
			for (int i = 0; i < maxLines; i++) {
				if (i == 0) {
					...
					System.out.printf("%-5d %-10d %-20s %-30s %-30s %-30s %10.2f %10.2f %10.2f%n", d.getSrNo(),
							d.getStudentId(), d.getStudentName(), safeGet(courses, i), safeGet(subjects, i),
							safeGet(teachers, i), d.getTotalFee(), d.getTotalPaid(), d.getPendingFee());
				} else {
